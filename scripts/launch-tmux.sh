...
tmux rename-window -t $SESSION:0 htb
tmux new-window -t $SESSION:1 -n staging
...
tmux send-keys -t $SESSION:0.1 "cd ${HOME}/toolkit" C-m
tmux send-keys -t $SESSION:0.1 "clear" C-m
tmux send-keys -t $SESSION:0.2 "cd ${HOME}/toolkit/transfers" C-m
...
tmux send-keys -t $SESSION:1.1 "cd ${HOME}/toolkit" C-m
tmux send-keys -t $SESSION:1.1 "clear" C-m
tmux send-keys -t $SESSION:1.2 "cd ${HOME}/toolkit" C-m
tmux send-keys -t $SESSION:1.2 "clear" C-m
#tmux select-layout tiled

tmux send-keys -t $SESSION:3 "cd ${HOME}/vpn/htb" C-m
tmux send-keys -t $SESSION:3 "sudo openvpn "

# Don't need window 2 - utility, but easier to kill it here rather than re-number everything
tmux kill-window -t $SESSION:2

echo -e "\n[DEBUG] Listing all windows and end of script"
